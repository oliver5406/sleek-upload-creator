...
const FileUploader = () => {
  ...
  const getInitialState = () => {
    try {
      ...
      if (savedState) {
        ...
        if (parsedState && typeof parsedState === 'object' && (parsedState.batchId || parsedState.files?.length > 0)) {
          return {
      ...
      return { batchId: null, processingComplete: false, files: [], progress: 0 };
    } catch (e) {
  ...
  useEffect(() => {
    if (batchId || files.length > 0) {
      const stateToSave = { batchId, processingComplete, files, progress };
  ...
  const updateFilePrompt = useCallback((id: string, prompt: string) => {
    setFiles(prev => prev.map(file => file.id === id ? { ...file, prompt } : file));
  }, []);
  ...
  const uploadFiles = useCallback(async () => {
    if (files.length === 0) {
      toast({ title: "No files selected", description: "Please add at least one property image to proceed.", variant: "destructive" });
      return;
    }
    const missingPrompts = files.some(file => !file.prompt);
    if (missingPrompts) {
      toast({ title: "Missing prompts", description: "Please add a prompt for each image.", variant: "destructive" });
      return;
    }
    stopPolling();
    toastShownRef.current = false;
    ...
    try {
      ...
      if (!token) {
        toast({ title: "Authentication failed", description: "You are not authenticated. Please log in again.", variant: "destructive" });
  ...
  const pollBatchStatus = useCallback(async (currentBatchId: string) => {
    ...
    try {
      ...
      if (batchStatus.job_details && batchStatus.job_details.length > 0) {
        const totalProgress = batchStatus.job_details.reduce((sum, job) => sum + (job.progress || 0), 0);
      ...
      if (isFinished) {
        ...
        if (!toastShownRef.current) {
          if (status === 'completed') {
            toast({ title: "Property video created!", description: "Your property tour video is ready to download" });
          } else if (status === 'failed' || status === 'error') {
            toast({ title: "Processing failed", description: "There was an error processing your video.", variant: "destructive" });
          } else if (status === 'partially_completed') {
            toast({ title: "Processing partially completed", description: "Some videos were processed successfully, others failed." });
          }
          toastShownRef.current = true;
        }
      } else {
        pollTimeoutRef.current = setTimeout(() => { pollBatchStatus(currentBatchId); }, statusPollingInterval);
      }
    } catch (error) {
      console.error('Status polling error:', error);
      setHasError(true);
      if (!toastShownRef.current) {
        toast({ title: "Status check failed", description: "Failed to check processing status.", variant: "destructive" });
        toastShownRef.current = true;
      }
      stopPolling();
      setIsUploading(false);
    ...
    try {
      ...
      fetch(downloadUrl, { headers: { 'Authorization': `Bearer ${token}` } })
        ...
        .then(blob => {
          ...
          toast({ title: "Download started", description: "Your property videos are being downloaded." });
        })
        .catch(err => {
          console.error('Download error:', err);
          setHasError(true);
          toast({ title: "Download failed", description: "There was an error downloading your video.", variant: "destructive" });
        });
    } catch (error) {
      console.error('Token retrieval error:', error);
      setHasError(true);
      toast({ title: "Authentication error", description: "Failed to authenticate for download.", variant: "destructive" });
    }
  ...
  useEffect(() => {
    const validateSavedState = async () => {
      try {
        ...
        if (['completed', 'failed', 'error', 'partially_completed'].includes(status)) {
          setProcessingComplete(true);
        ...
        <>
          {((!batchId || !processingComplete || hasError) && files.length === 0) && (
            <FileDropZone 
              ...
              currentFileCount={files.length}  {/* 🆕 ADD THIS LINE */}
            />
          )}
